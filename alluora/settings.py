...
# Set on Bunny library Security tab when Token Authentication is enabled.
# When set, hls_url() / thumbnail_url() return signed URLs that expire
# after BUNNY_STREAM_URL_TTL seconds. Leave empty to disable signing.
BUNNY_STREAM_TOKEN_AUTH_KEY = config('BUNNY_STREAM_TOKEN_AUTH_KEY', default='')
BUNNY_STREAM_URL_TTL = config('BUNNY_STREAM_URL_TTL', default=6 * 3600, cast=int)
...
REST_FRAMEWORK = {
    'DEFAULT_AUTHENTICATION_CLASSES': [
        # Token first — fastest, no WP roundtrip. Falls through to the
        # WP cookie class on the first request (when no token exists)
        # or on logout / token rotation.
        'apps.accounts.authentication.AlluoraTokenAuthentication',
    ...
    # Per-scope throttles — apps opt in by setting a throttle_classes
    # subclass with `scope = '<key>'`. Skin AI analyze is gated to a
    # generous 30/hour so an accidental retry loop or a malicious
    # client can't drain a user's Glow Coin balance or pin the
    # DeepFace CPU.
    'DEFAULT_THROTTLE_CLASSES': [],
    'DEFAULT_THROTTLE_RATES': {
        'skinai_analyze': '30/hour',
        # Public healthcheck — generous but bounded so a misconfigured
        # monitor or a noisy bot can't pin DeepFace CPU.
        'skinai_health': '60/minute',
        # Daily check-in — once per day per user. Low enough that
        # check-ins can't substitute for paid full scans, high enough
        # for the casual progress-tracking use case.
        'skinai_check_in': '1/day',
    },
}
...
# ---------------------------------------------------------------------------
# Minimum scans-per-tier before tier_averages() returns numbers. Below
# this floor the helper returns {} and the serializer's
# tier_comparison field stays empty — the in-app + admin UIs hide the
# "vs <tier> average" block entirely. Bump the env var
# SKINAI_TIER_AVG_MIN_USERS once each tier has enough recent activity
# (default of 10 keeps the first user's score from masquerading as an
# average).
SKINAI_TIER_AVG_MIN_USERS = config(
    'SKINAI_TIER_AVG_MIN_USERS', default=10, cast=int,
)
# How long to cache the per-tier average computation. Aggregation
# touches every recent scan; refresh hourly is plenty for a metric
# that moves on a population scale.
SKINAI_TIER_AVG_CACHE_TTL = config(
    'SKINAI_TIER_AVG_CACHE_TTL', default=3600, cast=int,
)
...
# Railway volume mounts at /data/media in production so Skin AI
# thumbnails survive redeploys. Locally /data doesn't exist, so fall
# back to BASE_DIR/media. Override with MEDIA_ROOT env var if needed.
MEDIA_URL = '/media/'
_default_media_root = (
    '/data/media' if os.path.isdir('/data') else str(BASE_DIR / 'media')
)
MEDIA_ROOT = config('MEDIA_ROOT', default=_default_media_root)
...
# ---------------------------------------------------------------------------
# Activates only when SENTRY_DSN is set; silent no-op in dev so local
# runs do not need a real DSN. The Django integration captures
# unhandled exceptions in views, signals, and the Skin AI pipeline.
